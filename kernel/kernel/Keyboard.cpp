...
namespace Keyboard
{
	...
	static PinP::Queue<Command>	s_keyboard_command_queue;
	static uint8_t			s_keyboard_command_extra = 0x00;

	static PinP::Queue<KeyEvent> s_key_event_queue;
	...
	bool initialize(void (*callback)(KeyEvent))
	{
		...
		{
			derrorln("PS/2 controller self test failed\e[m");
			return false;
		}
		...
		{
			derrorln("PS/2 first port test failed\e[m");
			return false;
		}
