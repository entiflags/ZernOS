...
/* searches for the lowest set bit in 32-bit value, value must not be 0 */
...
/* searches for the highest set bit in 32-bit value must not be 0 */
...
BaseAdressRegister PeripheralComponentInterconnectController::GetBaseAdressRegister(uint16_t bus, uint16_t device, uint16_t function, uint16_t bar) {
    ...
    if(result.type == MemoryMapping){
            ...
            case 0: // 32 Bit Mode
        {
            ...
            const uint32_t barValue = Read(bus,device,function,barOffset);  // and read back again
            if (barValue == 0)  // at least one address bit must be 1 (i.e. writeable).
            {
                if (result.preFetchable)    //unused BARs must be completely 0 (the prefetchable bit most not be set either)
                {
