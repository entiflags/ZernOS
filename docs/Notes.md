...
- - Last Twelve: Ram Number (MUST BE MULTIPLE OF 4)

# Graphics
Here are the notes on graphics for the operating system. (May need to read hardware communication first)
### Graphics Mode {VGA}
- To write pixels and such to the screen for a GUI graphics card needs to be put into graphics mode (grub puts it into text mode by default)
...
- To set the graphics mode via the GPU defiierent sets of values have to be sent to different places, for example a 320px wide by 200px height x 8-bit colour depth graphics mode would have to be set by sending these values:
- - Misc Port: 0x63,
